...
@export_group("Sensor C++ Node")
@export var sensor: Node
...
func get_obs() -> Dictionary:
	#var obs := sensor.get_8_wayVision(get_parent().position, get_parent().map, 16)
	#var dir := global_position.direction_to(get_parent().player)
	#return {"obs": []}
	return Dictionary()
